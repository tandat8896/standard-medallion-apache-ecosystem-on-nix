#!/usr/bin/env python3
"""
spark_gold_to_postgres_daily_revenue.py

Loads Gold daily_revenue KPI from Parquet into PostgreSQL.

Usage:
    PROCESSING_DATE=2026-04-17 spark-submit \
        --master spark://127.0.0.1:7077 \
        scripts/spark_gold_to_postgres_daily_revenue.py
"""
...
def read_gold_daily_revenue(spark: SparkSession, date: str) -> DataFrame:
    """Read daily_revenue from Gold Parquet layer."""
    path = f"{GOLD_ROOT}/kpi=daily_revenue/dt={date}"
    logger.info(f"Reading Gold daily_revenue from: {path}")

    df = spark.read.parquet(str(path))
    count = df.count()
    logger.info(f"Loaded {count} revenue records")
...
def transform_for_postgres(df: DataFrame) -> DataFrame:
    ...
    # Rename _aggregated_at to aggregated_at (remove underscore)
...
def run_gold_to_postgres_daily_revenue(processing_date: str) -> None:
    logger.info("=" * 70)
    logger.info("Starting Gold → PostgreSQL ETL for kpi_daily_revenue")
    logger.info(f"Processing date: {processing_date}")
    logger.info("=" * 70)

    spark = (
        SparkSession.builder
        .appName(f"Gold2Postgres_DailyRevenue_{processing_date}")
        .master("spark://127.0.0.1:7077")
...
if __name__ == "__main__":
    run_gold_to_postgres_daily_revenue(PROCESSING_DATE)
